...
export const stringify = <TData = unknown>(data: TData) =>
  serialize(data, function innerReplacer(this: any, key) {
    ...
    if (directValueReference instanceof Date) return { __type: "Date", value: directValueReference.toISOString() };
  });
...
export const createPersistConfig = ({ queryClient }: { queryClient: QueryClient }): PersistQueryClientOptions => ({
  queryClient,
  persister: persister(),
  dehydrateOptions: {
