...
import { ChildFuseReferenceType, ChildFuses, ParentFuseReferenceType, ParentFuses } from "@ensdomains/ensjs/utils";
...
export interface Transaction<TData> {
  displayItems: (data: TData) => TransactionDisplayItem[];
  transaction: (
...
export type ExtractTransactionData<TTransaction> = TTransaction extends Transaction<infer TData> ? TData : never;
