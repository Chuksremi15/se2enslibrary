...
} from "~~/enslibrary/components/@molecules/Register/types";
...
export const getSelectedIndex = (state: RegistrationReducerData, selected: SelectedItemProperties) =>
...
const useRegistrationReducer = ({ address, name }: { address: string | undefined; name: string }) => {
  const chainId = useChainId();
  const selected = { address: address!, name, chainId } as const;
  const [state, dispatch] = useLocalStorageReducer<RegistrationReducerData, RegistrationReducerAction>(
    "registration-status",
    reducer,
    { items: [] },
  );
